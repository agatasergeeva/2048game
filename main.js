(function () {
  ...
  function emptyCells(){
    ...
    for(let y=0;y<SIZE;y++) 
      for(let x=0;x<SIZE;x++) 
        if(state.grid[y][x]===0) arr.push({x,y});
    return arr;
  }
  ...
  function snapshot(){
    return { 
      ...
      tiles: JSON.stringify(Array.from(tilesMap.entries())) 
    };
  }
  ...
  function move(dir){
    ...
    const newGrid=[]; 
    for(let y=0;y<SIZE;y++){ 
      const {result,scoreGain} = lineMove(g[y]); 
      newGrid.push(result); 
    ...
    for(let y=0;y<SIZE;y++){
      for(let x=0;x<SIZE;x++){
        ...
        if(
          origins.length===2 && 
          origins[0].t.value===origins[1].t.value && 
          origins[0].t.value*2===v
        ){
          tileId = state.idCounter++; 
          mergesToAnimate.add(tileId);
  ...
  $$('.tab').forEach(t=>t.addEventListener('click', ()=>{
    $$('.tab').forEach(z=>z.classList.remove('active')); 
